...
if (!defined('PATH_ROOT')) {
	...
	define('PATH_ROOT', ePath::clean(PATH_SITE));
}
...
class ePath
{
		...
		if (is_dir($path))
		{
			...
			while ($file = readdir($dh))
			{
				if ($file != '.' && $file != '..') {
					$fullpath = $path.'/'.$file;
					if (is_dir($fullpath)) {
						if (!ePath::setPermissions($fullpath, $filemode, $foldermode)) {
							$ret = false;
						}
					} else {
						if (isset ($filemode)) {
	...
	function check($path)
	{
		if (strpos($path, '..') !== false) {
			eError::raiseError( 20, 'ePath::check Use of relative paths not permitted'); // don't translate
			eexit();
		}
		$path = ePath::clean($path);
		if (strpos($path, ePath::clean(PATH_ROOT)) !== 0) {
			eError::raiseError( 20, 'ePath::check Snooping out of bounds @ '.$path); // don't translate
			eexit();
		}
	}
	...
	function isOwner($path)
	{
		...
		eimport('light-error.helper');

		$tmp = md5(JUserHelper::genRandomPassword(16));
		$ssp = ini_get('session.save_path');
		$jtp = PATH_SITE.DS.'tmp';
