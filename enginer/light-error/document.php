...
eLoader::register('eDocumentRenderer', dirname(__FILE__).DS.'renderer.php');
 ...
 * @since		1.5
 */
class eDocument extends eObject
{
	/**
	 * Document title
	 *
		...
		if (empty($instances[$signature]))
		{
			...
			if(!class_exists($class))
			{
				...
				if (file_exists($path)) {
					...
					eError::raiseError(500,eText::_('Unable to load document class'));
				}
			}
	...
	*/
	function &loadRenderer( $type )
	{
		...
		if( !class_exists( $class ) )
		{
			...
			if(file_exists($path)) {
				...
				eError::raiseError(500,eText::_('Unable to load renderer class'));
			}
		}
	...
	{
		eResponse::setHeader( 'Expires', gmdate( 'D, d M Y H:i:s', time() + 900 ) . ' GMT' );
		if ($mdate = $this->getModifiedDate()) {
			JResponse::setHeader( 'Last-Modified', $mdate /* gmdate( 'D, d M Y H:i:s', time() + 900 ) . ' GMT' */ );
		}
		eResponse::setHeader( 'Content-Type', $this->_mime .  '; charset=' . $this->_charset);
	}
}
