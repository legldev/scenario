...
class eArrayHelper
{
	...
	function toInteger(&$array, $default = null)
	{
		if (is_array($array)) {
			...
			if ($default === null) {
				...
				eArrayHelper::toInteger($default, null);
				$array = $default;
			} else {
	...
	function toObject(&$array, $class = 'stdClass')
	{
		...
		if (is_array($array))
		{
			...
			{
				if (is_array($v)) {
					$obj->$k = eArrayHelper::toObject($v, $class);
				} else {
					$obj->$k = $v;
				}
			}
		...
		if (is_array($array))
		{
			foreach ($array as $key => $item)
			{
				if (is_array ($item))
				{
					...
					$output[] = eArrayHelper::toString( $item, $inner_glue, $outer_glue, $keepOuterKey);
				}
				else {
		...
		if (is_object( $p_obj ))
		{
			...
			foreach (get_object_vars($p_obj) as $k => $v)
			{
				...
				if (is_object( $v ))
				{
					if ($recurse)
					{
						$result[$k] = eArrayHelper::fromObject( $v, $recurse, $regex );
					}
				}
				else
				{
	...
	{
		$GLOBALS['ENG_so'] = array(
			'key'		=> $k,
			'direction'	=> $direction
		);
		usort( $a, array('eArrayHelper', '_sortObjects') );
		unset( $GLOBALS['ENG_so'] );
