...
class eRequest
{
	...
	function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
	{
		...
		if (isset($GLOBALS['_REQUEST'][$name]['SET.'.$hash]) && ($GLOBALS['_REQUEST'][$name]['SET.'.$hash] === true)) {
			...
			$var = eRequest::_cleanVar($var, $mask, $type);
		}
		elseif (!isset($GLOBALS['_REQUEST'][$name][$sig]))
		{
			if (isset($input[$name]) && $input[$name] !== null) {
				// Get the variable from the input hash and clean it
				$var = eRequest::_cleanVar($input[$name], $mask, $type);

				// Handle magic quotes compatability
				if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
					$var = eRequest::_stripSlashesRecursive( $var );
				}
			...
			elseif ($default !== null) {
				...
				$var = eRequest::_cleanVar($default, $mask, $type);
			}
			else {
				$var = $default;
			}
	...
	{
		return eRequest::getVar($name, $default, $hash, 'int');
	}
	...
	{
		return eRequest::getVar($name, $default, $hash, 'float');
	}
	...
	{
		return eRequest::getVar($name, $default, $hash, 'bool');
	}
	...
	{
		return eRequest::getVar($name, $default, $hash, 'word');
	}
	...
	{
		return eRequest::getVar($name, $default, $hash, 'cmd');
	}
	...
	function getString($name, $default = '', $hash = 'default', $mask = 0)
	{
		...
		return (string) eRequest::getVar($name, $default, $hash, 'string', $mask);
	}
	...
	function get($hash = 'default', $mask = 0)
	{
		...
		$result = eRequest::_cleanVar($input, $mask);

		// Handle magic quotes compatability
		if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
			$result = eRequest::_stripSlashesRecursive( $result );
		}
	...
	function set( $array, $hash = 'default', $overwrite = true )
	{
		foreach ($array as $key => $value) {
			eRequest::setVar($key, $value, $hash, $overwrite);
		}
	}
	...
	{
		$token	= eUtility::getToken();
		if(!eRequest::getVar( $token, '', $method, 'alnum' )) {
			$session = eFactory::getSession();
			if($session->isNew()) {
				//Redirect to login screen
				global $mainframe;
				$return = eRoute::_('index.php');
;				$mainframe->redirect($return, eText::_('SESSION_EXPIRED'));
				$mainframe->close();
			} else {
				return false;
			}
	...
	function clean()
	{
		eRequest::_cleanArray( $_FILES );
		eRequest::_cleanArray( $_ENV );
		eRequest::_cleanArray( $_GET );
		eRequest::_cleanArray( $_POST );
		eRequest::_cleanArray( $_COOKIE );
		eRequest::_cleanArray( $_SERVER );

		if (isset( $_SESSION )) {
			eRequest::_cleanArray( $_SESSION );
		}
	...
	function _cleanArray( &$array, $globalise=false )
	{
		...
		foreach ($array as $key => $value)
		{
			// PHP GLOBALS ineection bug
			$failed = in_array( strtolower( $key ), $banned );

			// PHP Zend_Hash_Del_Key_Or_Index bug
			$failed |= is_numeric( $key );
			if ($failed) {
				eexit( 'Illegal variable <b>' . implode( '</b> or <b>', $banned ) . '</b> passed to script.' );
			}
			if ($globalise) {
				$GLOBALS[$key] = $value;
			}
	 ...
	 * @param string The variable type {@see eFilterInput::clean()}.
	 */
	function _cleanVar($var, $mask = 0, $type=null)
	{
		...
		elseif ($mask & 4)
		{
			...
			if (is_null($safeHtmlFilter)) {
				$safeHtmlFilter = & eFilterInput::getInstance(null, null, 1, 1);
			}
		...
		else
		{
			...
			if (is_null($noHtmlFilter)) {
				$noHtmlFilter = & eFilterInput::getInstance(/* $tags, $attr, $tag_method, $attr_method, $xss_auto */);
			}
	...
	{
		$value = is_array( $value ) ? array_map( array( 'eRequest', '_stripSlashesRecursive' ), $value ) : stripslashes( $value );
		return $value;
	}
}
